...
Каждая агент-сессия поднимает **свой** Vite и **сама его гасит**. Чужой сервер не шарить — так ломается чужой дебаг (HMR, `GAME` state, вкладка chrome-devtools).

Порт не хардкодить. `strictPort` не включать. `--port 5173` не форсить. Соседние проекты (PixisEditor) и `start-dev-server.bat` пользователя не трогать.

### Старт

1. Всегда `npm run dev` **для этой сессии**. Не подключаться к уже крутящемуся Vite, даже если cwd = это репо.
2. Vite сам берёт свободный порт (5173, 5174, …). URL **только** из stdout (`Local: http://localhost:NNNN/`), не угадывать.
3. Запомнить PID и порт. Вывод — в `.dev-server.<port>.log` (свой файл, не общий `.dev-server.log`).
4. Браузер: открывать **только этот** URL. `list_pages` — если вкладка the-ledge на **другом** порту (чужая сессия / пользователь) → не `select_page`. Свою вкладку создать/navigate на свой URL.

### Стоп (обязательно — не оставлять мусор)

После browser verification и в конце задачи, если сервер поднимали:

1. Закрыть вкладку, которую **эта** сессия открыла. Чужие вкладки не трогать.
2. Убить **свой** PID. Чужие `node`/`vite` (другие сессии, Pixis, `start-dev-server.bat`) не гасить.
3. Удалить свой `.dev-server.<port>.log`.
4. Если пользователь сказал «оставь сервер» — не гасить.

Не плодить хвосты на 5174/5175/…. Сессия без browser-tier сервер не поднимает.

### Zombie-check

Перед стартом: зависшие vite **этой же сессии** после краша — можно убить по своему PID/логу. Чужой PID — только по явной просьбе пользователя. Массовое убийство процессов — подтверждение обязательно.
...
1. Поднять **свой** Vite (см. Dev server). URL только из stdout.
2. `list_pages`. Своя вкладка на **своём** порту → `select_page`. Чужую the-ledge вкладку (другой порт / чужая сессия) не занимать.
3. Своей вкладки нет → `navigate_page` на свой URL. Не угадывать `:5173`.
4. После проверки закрыть свою вкладку и убить свой сервер (см. Dev server / Стоп).
