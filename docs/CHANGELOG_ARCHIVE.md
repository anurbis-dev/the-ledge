...
## 2026-09-03 (pre-commit, unreleased at commit 9daa8ea)
...
## 2026-09-03 (pre-commit, unreleased at commit 04ec722)

- Fix(render): зум в редакторе (`ED.zoom`/`viewScale` < 1, отдаление) — `clearRect`/`fillSky` чистили и красили только фиксированные `BUF_W×BUF_H`/`VW×VH` мировых юниц вместо реально видимой области `viewW()×viewH()`, из-за чего края канваса не перекрывались новым кадром и старые тайлы/фон копились слоями при каждом зуме. Добавлен `clearFrame()` (`render/ctx.js`, делит на `viewScale`) вместо ручного `ctx.clearRect(0,0,BUF_W,BUF_H)` в `app/loop.js`; `fillSky`/`drawStars`/`ridge` (`render/fx.js`) переведены на `viewW()`/`viewH()`.
...
## 2026-09-02 (pre-commit, unreleased at commit 13f1db3)
...
## 2026-09-02 (pre-commit, unreleased at commit 7256b2c)
...
## 2026-09-01 (pre-commit, unreleased at commit a29049f)
...
## 2026-09-01 (pre-commit, unreleased at commit 21d8bd9)
...
## 2026-09-01 (pre-commit, unreleased at commit eb10fc2)
...
## 2026-09-01 (pre-commit, unreleased at commit ebe9380)
...
## 2026-09-01 (pre-commit, unreleased at commit 5795319)
...
## 2026-09-01 (pre-commit, unreleased at commit e991dcd)

- Fix(physics): хват руками за низ вертикальной лестницы (`autoLadder`, добавлен в предыдущем фиксе) якорился на верхней грани текущей клетки перекладины вместо нижней — расходилось с уже существующими точками хвата (`tryGrab`/«низ висячей лестницы», ручной сход вниз в `updateLadder`), из-за чего поза виса визуально не совпадала с местом хвата. Приведено к единой линии — нижняя грань клетки, как у остальных мест. Также откачен вчерашний «висит и сразу падает по удержанному ↓»: отпускание с виса на лестнице снова только по свежему нажатию ↓ (`downPressed`), не по удержанию — так герой сперва повисает на руках, и падает только повторным нажатием.
...
## 2026-08-28 (pre-commit, unreleased at commit a5e33f1)

- Fix(physics): автозацеп за вертикальную лестницу в падении (`autoLadder`) полностью замонтировывал в активный лаз (`state:'ladder'`) даже у самого низа колонны, где рядом и так есть пол — лишний рывок вместо приземления. Теперь вертикальная (не диагональная) лестница в падении цепляет только руками (`grabTo`/`hang`), как и обычный хват кромки; диагональные лестницы-настилы не тронуты. Заодно вис на нижней перекладине над пропастью не отпускался по удержанному ↓ (`updateHang` слушал только `downPressed`-фронт, который уже был израсходован спуском по лестнице) — висела бесконечно без повторного нажатия. Для `hang.kind==='lad'` держание ↓ теперь тоже отпускает, с коротким `ladCd`, чтобы `autoLadder` не перехватил обратно тем же кадром.
- Fix(physics): на пологих скосах над педесталом привязка к нижней грани всё ещё съезжала на пик — `edgeLand` (посадка впритык к грани, без `STAND_OFF` вглубь) не имел допуска, а даже впритык самое начало диагонали перекрывает низ бокса на пару px. Добавлен тот же STEP_UP-допуск, что у `setStance`/`moveX` (пробуем y, y-1…y-STEP_UP); порядок попыток в `findWallTop` тоже поменян — сперва впритык к грани, и только если совсем не влезает (крутой уклон, бокс шире зазора) — обычный `bestSlopeLand` со сдвигом вглубь.
- Fix(physics): `tryClimbWall` (упор+вверх на стену выше 1 тайла) при скосе-крышке над педесталом лез сразу на пик скоса, игнорируя, что его нижняя грань уже на высоте педестала. `findWallTop` теперь пробует посадку на каждом ряду снизу вверх и берёт первый, где реально влезает без наложения на диагональ (для рядов под скосом — ещё и впритык к грани, без обычного отступа `STAND_OFF` вглубь, который на скосе сразу толкает бокс в подъём). На настоящем крутом уклоне (45°, «2:1») бокс физически шире зазора у низкого края скоса — там залезание на тайл выше и остаётся (геометрический предел, не баг); на пологих уклонах привязка к нижней грани теперь работает.
- Feat(physics): упор в стену выше 1 тайла + вверх теперь тоже лезет наверх с земли, без прыжка — раньше подъём на такую стену шёл только через прыжок+хват в воздухе (`tryGrab` не работает стоя). Новый `tryClimbWall`/`findWallTop` (`player.js`) сканирует сплошную колонку вверх от ног (как `findChestStep` у mantle, но без потолка «ровно 1 тайл»), предел высоты — `C.CLIMB_WALL_TILES` (дефолт 3, Params → Grab/Climb), подобран так, чтобы не превышать то, что и так достижимо обычным прыжком. Верхний тайл стены может быть скосом (посадка через `bestSlopeLand`).
