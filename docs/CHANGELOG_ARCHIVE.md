...
## 2026-09-03 (pre-commit, unreleased at commit a7d1dea)
...
## 2026-09-03 (pre-commit, unreleased at commit 80509fb)
...
- Fix(render,entities): `tryDismount` парковал транспорт и сразу переставлял его на позицию героини, пока 0.25с ещё доигрывала unmount-анимация (герой рисуется "как транспорт" через `tryVehicleSprite`) — на это окно `vehicles()` рисовал тот же транспорт второй раз. Добавлен `p.mountAnimVehicle`, скрывающий его из `vehicles()` на время анимации.
- Fix(render): `tryVehicleSprite` зеркалил кадр транспорта через `blitHeroSprite` (пивот `ox+fx`, подобранный под кадр героини) — для транспорта с `fx:0` бокс при facing<0 уезжал на всю ширину кадра. Переведён на `blitEntSprite` (тот же пивот, что и у запаркованного транспорта), сохраняющий бокс по обеим сторонам.
- Fix(render): анимация движения в маунте была жёстко `move` — для скина-транспорта на геройском спрайте (family `hero`, нет слота `move`, только `run`) кадр не находился и рисовался фоллбэк-бокс. `vehicleMoveAnim` теперь берёт `move`, только если он есть у def, иначе `run`, как у обычного персонажа.
