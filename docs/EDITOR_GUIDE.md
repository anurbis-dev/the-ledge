...
- `Door`: всегда пара — **2 клика** (`mkDoorAt`); первый ждёт return (`doorPending`), второй связывает `pair` по id. `Esc` отменяет первый (удаляет pending). `Delete` снимает **оба** конца пары. Поля `need` / `consume` / `locked=!!need` пишутся в persist.
