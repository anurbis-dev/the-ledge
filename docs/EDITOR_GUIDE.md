...
- `0` сброс зума в `1:1`.
- `X` / `Y` (пока нажаты) отразить кисть горизонтально / вертикально. Предпросмотр в edDrawOverlay, применяется при рисовании тайла/дек. Флип хранится в per-tile `flip` Uint8Array (бит0=H, бит1=V) в базовой карте (`runtime.flip`), a не только в слоях редактора — при сохранении (Bake) и экспорте текста flip-данные сохраняются. Стирание клетки сбрасывает flip; копия/паста/undo переносят flip. **Для кисти скосов**: X (горизонтальный флип) подменяет id через `mirrorSlopeId` (без изменений); Y (вертикальный флип) теперь активно работает и превращает скос в потолочный (диагональный нависающий оверхэнг), о который игрок ударяется головой при движении вверх.
