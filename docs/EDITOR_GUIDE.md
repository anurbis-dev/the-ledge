...
- `0` сброс зума в `1:1`.
- `X` / `Y` (пока нажаты, можно оба одновременно) отразить кисть горизонтально / вертикально. Предпросмотр в edDrawOverlay, применяется при рисовании тайла/дек через общий per-cell канал `flip` (бит0=H, бит1=V) в `layers.js`/`game.js` (`setFlip`/`flipAt`) — у скосов тоже, включая X (раньше X на скосе подменял id через `mirrorSlopeId`, отчего пропадал спрайт у id без своей картинки; теперь один и тот же id просто зеркалится канвасом + геометрией через `slopeSpec(v,fl)`). Стирание клетки сбрасывает flip; копия/паста/undo переносят flip. Y на скосе зеркалит геометрию по вертикали и переносит сплошную часть на другую грань тайла: без Y сплошное — низ (гарантированная грань — нижняя, стоим на диагонали сверху), с Y сплошное — верх (гарантированная грань — верхняя, стоим на плоском верху тайла, а диагональ снизу работает как нависающий потолок — бьёмся об неё головой при движении вверх). Обе ориентации теперь блокируют AABB как обычный блок по своей сплошной грани (`slopeBlocks` в `map.js`) — раньше блокировала только Y-версия, из-за чего в обычный (не-Y) скос можно было провалиться снизу.
...
- Details (`#edTileEdit`, `fillTileParamsOnly`): слот **Sprite** + collision/flags/params only. Дроп `{ spriteId }` → `setTileSpriteId` (custom → `tile.spriteId`, builtin → `tileGfx.spriteId`). Clear / Edit / dblclick слота → `openSpriteEdit` (спрайт открывается в режиме Paint). Нет Paint / Re-import / Reset picture в самом окне тайла — пиксели только на **Sprites**. Resolve: `getTileSpriteId` / `tileFrameSrc` / `tileImage` берут idle (или первый anim) спрайта; иначе legacy `src`/`frames`. Boot/`Ctrl+D`/PNG-drop мигрируют picture → sprite (`migrateTilePicture`). Thumbs: `tileThumb` учитывает `spriteId`.
...
- Каталог `SPRITE_DEFS`: персонажи (hero / enemy* / flier* / spider* / npc_*) **и** placeable object icons (coin, chest, door, torch, gear, markers, ropes/plats/lift, …). Персонажи без авто-bake иконок; icon-каталог при boot получает dirty иконки через `ensureCatalogIconFrames` (`migrate-graphics.js` → теперь бейкает **все анимации** спрайта, не только первую → `paintObjIcon` / `bakeSpriteFrameSrc`); для альт-анимаций (open/closed/…) `bakeKindFrame` добавляет суффикс `_<animId>` к paintKind (chest + open → chest_open, lift + open → lift_open) — каждому состоянию свой плейсхолдер-арт в thumbs.js. Hero-family спрайты при первом открытии sprite-edit для ещё не нарисованных кадров автоматически бейкают стартовый растровый силуэт процедурной позы через `bakeHeroFrame()`.
- Даблклик свача → `openSpriteEdit` (кадры / пиксели в `#edTileEdit`; якоря — только в Object Details). Окно Details при открытии из Sprites-палитры содержит поля **Name** и **Tag** (builtin и custom; Enter или F2 на сватче). Для custom спрайтов Tag пишется в snapshot; для builtin — dev-draft localStorage `ledge.ed.spriteTags`. В панели **Size/Box**: контрол **Res** (выпадающий 1×/2×/3×/4×, дефолт 1×) для множителя разрешения пикселей; холст редактора и импорт PNG масштабируются до `fw*res × fh*res` нативных пикселей, но якоря и мировой футпринт `fw×fh` не меняются.
- Драг kind `sprite` → payload `{ spriteId }` на Tile/Object **Sprite** slot или на thumb кадра action (в sprite-mode).
- Drop PNG на вкладку → `addSpriteDef` (не `addTile`); широкий шит → idle frames; размер PNG определяет fw/fh, импортируемый PNG может быть в любом разрешении (масштабируется до fw×fh при рендере).
- `Ctrl+D` → `cloneSpriteDef` («… copy»), открывает клон; `res` прокидывается в клон.
...
- `Door` (`kind: 'door'`) — парная дверь (warp между двумя точками). Рисуется спрайтом 'door' (если запечён, иначе процедурно); objectKind 'door_env' для якорей в Object Details.
- Враги/птицы/пауки/щупальца — Details = params + Sprite slot + якоря; кадры через Edit / Sprites (не авто-Paint). Клик по врагу открывает плавающее окно **Enemy AI** (`src/editor/enemy-settings.js`) с секцией лута (иконка/имя/qty/удаление, random-тумблер если лута >1), слайдером Walk speed, списком **Patrol points** (каждая точка: «Point N» + поле pause-задержки + кнопка удаления; минимум 2 точки; кнопка «+ Point»), тумблером Can chase и (при включении) слайдерами Chase speed / Sight range / Hear range. Позиция каждой точки редактируется перетаскиванием маркера прямо на карте: при выборе врага на канве видна пунктирная линия между min/max points с ручками в каждой точке (оранжевые если canChase включён, зелёные если нет); dragging ручки двигает точку. Delete при выбранном враге удаляет врага; выбор врага же показывает гизмо.
- Предметы и лут.
- Сундуки (`Chest`, `Locked`) — рисуются спрайтами 'chest' (открытый/закрытый, анимации idle/open) или 'chestL' (заблокированный); objectKind 'chest_env' для якорей; падающий лут и подсказка про ключ остаются процедурными оверлеями поверх любого рендера.
...
- **Frame box** (чекбокс + Frame pos/Frame size под обычным Box) — per-frame override хитбокса именно этого кадра: позиция независима от Origin (в отличие от анимного Box, у которого позиция всегда = Origin), задаётся отдельными X/Y. Выкл. — кадр падает на анимный Box (как раньше); вкл. — сидирует текущим боксом и дальше редактируется своими полями. На канве такой бокс рисуется голубым (обычный — красным); драг-ресайз хитбокса мышью по-прежнему правит Origin+анимный Box, не Frame box — его редактируют только поля. Единственный потребитель — `updateClimb` (`player.js`) для `climb`-анимации героя: если у текущего кадра (по `climbFrameIndex(p.climb.p)`) есть Frame box, колижен ставится по нему (с учётом facing, тот же pinned-принцип, что у рендера виса/лаза), иначе — прежний линейный лерп from→to.
...
- `Enemy` (враги): клик по врагу открывает плавающее окно **Enemy AI** (`src/editor/enemy-settings.js`). Отображает: **Loot section** (иконка/имя/qty/× удаление для каждого предмета; random-тумблер если лута больше одного; пусто если лута нет), **Walk speed** (слайдер базовой скорости, из `.v`, 8–80 px/s), **Jump height** (слайдер 0–40px, дефолт 0; если > 0, враг прыгает через низкие препятствия при движении в patrol/chase вместо остановки; работает независимо от Can chase), **Patrol points** (список точек, minimum 2 — каждая точка содержит «Point N» + поле pause-задержки в секундах (0.1 шаг) + кнопка × удаления (disabled если points.length ≤ 2); кнопка «+ Point» добавляет новую точку справа от текущей последней на 40px), **Can chase** (тумблер, дефолт false, включает режим атаки), и при включении **Can chase** — **Chase speed** (20–160 px/s, дефолт v×1.6), **Sight range** (16–200px вперёд, дефолт 90), **Hear range** (0–160px назад, дефолт 50). Визуально на карте при выборе врага: пунктирная линия соединяет min/max points на уровне ног врага, каждая точка — маркер (ручка); цвет ручек: оранжевый если `canChase` включён, зелёный если нет. Перетаскивание ручки двигает точку по world X и вызывает `resyncEnemyRuntime` (сброс AI state, пересчёт x0/x1). Враг патрулирует пинг-понгом между точками с паузой на каждой; при `canChase` и видимости игрока переходит в chase на `chaseV`; сброс в patrol если игрок вышел за `sightFwd` или враг упёрся в границу/препятствие. Persist: `enemyAiCfg` пакуется только при отличии от дефолта (2 точки x0/x1, нет пауз, `canChase=false`, `jumpH=0`).
...
- Слайдер **Camera zoom** (25–400%, шаг 5, дефолт 100) — общий зум камеры уровня: насколько крупно рисуются мировые пиксели на экране. <100% = видно больше мира, >100% = крупнее пиксели/меньше обзор. Действует только в геймплее; редактор неподвижен на 320×180.
