...
- Спрайты (героиня, враги, птицы, пауки, NPC, фонарь света): секция Sprites в палитре Tiles, или даблклик по объекту / свачу. Клик по свачу выбирает кисть объекта, если она есть (враг/птица/паук/NPC/свет) — LMB или драг на канву ставит сущность. Даблклик всегда открывает правку кадров. Героиня только редактируется (спавн не дублируется). Каждая анимация — **одна строка кадров** (не куча). Полоса по умолчанию высотой в одну строку; сепаратор под ней тянется вниз, чтобы видеть больше рядов. СКМ-драг / колесо панорамируют строки без полосы прокрутки. Клик по кадру открывает его в том же пиксельном редакторе. Reset frame забывает правку — в игре снова старый рисунок. Пока кадр не красили, игра не переключается на спрайт.
