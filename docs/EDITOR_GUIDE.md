...
- `0` сброс зума в `1:1`.
- `X` / `Y` (пока нажаты) отразить кисть горизонтально / вертикально. Предпросмотр в edDrawOverlay, применяется при рисовании тайла/дек. Стирание клетки сбрасывает flip; копия/паста/undo переносят flip.
