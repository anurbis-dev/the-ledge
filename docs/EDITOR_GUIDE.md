...
- `Cover` — режим помещений: рисует, как клетка выглядит снаружи (пока героиня не внутри).
- `Bake` пишет полный снапшот в `src/core/defaults.js` через `POST /__bake`.
...
- `Ctrl + LMB`: пипетка (тайл/объект под курсором; в Cover — только cover-тайл).
...
- `Ctrl` при drag копирует объект и двигает копию.
