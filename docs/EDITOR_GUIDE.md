...
- `Ctrl+D` — дублировать **кисть тайла** с палитры Tiles в новый кастом-тайл (копия `src` / `frames` / флагов). Не выделение карты и не объекты (kinds не клонируются).
...
- Спрайты (героиня, враги, птицы, пауки, NPC, фонарь света): секция Sprites в палитре Tiles. Клик по свачу выбирает кисть объекта, если она есть (враг/птица/паук/NPC/свет) — LMB или драг на канву ставит сущность. Даблклик по свачу в палитре (не на канве) открывает `#edTileEdit`. Героиня только правится как спрайт; точка старта уровня — объект `Start` (`player_start`) во вкладке Objects. Каждая анимация — **одна строка кадров**. Полоса по умолчанию высотой в одну строку; сепаратор тянется вниз. СКМ-драг / колесо панорамируют строки. Клик по кадру — пиксельный редактор. `+` в конце ряда добавляет кадр (спрайты: `setAnimFrameCount`; тайлы: append в `frames[]`). Драг превью кадра меняет порядок в ряду. **Play** / Stop проигрывает текущую анимацию в окне (~8 fps). У `enemy*` / `flier*` / `spider*` при открытии bake-кадры материализуются во все слоты (dirty raster). Reset frame забывает правку — в игре снова старый рисунок. Пока кадр не красили (кроме материализации врагов), игра не переключается на спрайт.
...
Палитра включает:
- `Start` (`kind: 'player_start'`) — точка старта уровня.
...
## 8. Inspect и гизмо (Start / Sound / Light / Volume)

Выбор:
- Клик по объекту (включая маркер `Start`).
...
- Клавиша `Delete` (`Start` не удаляется — сброс `LV.spawn` на дефолт).
