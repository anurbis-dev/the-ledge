...
- Для `Fall` в Details (`#edTileEdit` / `fillTileParamsOnly`) — слайдеры процедурных струй (`paintFallStrands`, фаза по world Y через столбец): **Speed** (0–200, дефолт 70); **Length** (0–100, 25) — длина светлой нити (ярче сверху → плавно прозрачнее вниз); **Wave** (0–100, 15; 0 = прямая); **Random** (0–100, 35) — разброс phase/speed/x/length; **Offset** (0–100, 55; 0 = lockstep) — рассинхрон нитей; **Fade** (0–100, 0) — мягкая альфа body/нитей (без жёстких дырок), гасит fill и верхнюю пену. Meta: `tileGfx[14].speed` / `length` / `wave` / `random` / `offset` / `fade`.
