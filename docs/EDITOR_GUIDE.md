...
- Кастом-тайл: даблклик → пиксели (Paint / Erase / Pick, палитра цветов с тайла) + overlay / front / climb / one-way / collision. Красный контур = хитбокс героя, картинка его не задаёт. Hit или Collision → Custom box. Re-import PNG / дроп на окно заменяет `src`, id и флаги те же; широкий шит становится кадрами анимации. Встроенный тайл: Paint пишет спрайт-оверрайд (`tileGfx`), Reset picture снимает его.
- Героиня / враги: внизу палитры Tiles секция Sprites. Каждая анимация — своя строка кадров. Клик по кадру — правка пикселей. Неправленные кадры в игре рисуются как раньше.
