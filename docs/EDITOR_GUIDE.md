...
- `Ctrl + drag` на тайловом слое: рамка выделения. Потом LMB внутри — перенос куска, `Delete` стирает.
- Даблклик по тайлу (палитра или канва): окно имени, overlay, коллизии, переднего плана.
...
- Кастом-тайл: даблклик → overlay / front / climb / one-way / collision (none, full, half, bar, slope, custom box). Built-in коллизию не меняют.
...
- Связная область cover-клеток = одна комната. Героиня внутри (ноги на клетке) — видны настоящие тайлы; вышла — снова cover.
...
Плавающие окна: Layers, Inspect, Chest Loot, NPC Talk, Boulder Settings.
