...
- Для `Fall` в Details — слайдер **Speed** (0–200, дефолт 70): скорость процедурных струй; пишется в `tileGfx[14].speed`.
- Для `Water` в Details — **Shift** (−100…100, дефолт 100 = канон вправо; 0 = стоячая база): общий дрейф реки; **Wave X** (0–100, дефолт 50): амплитуда гребня/bob. `Fall` сверху/с края гасит дрейф рядом и даёт **расходящиеся в обе стороны** гребни `sin(k·|x−src|−ωt)` с falloff. Вход/выход героя из воды — кольца ряби (`addWaterRipple`). Meta в `tileGfx[13].shift` / `waveX`.
