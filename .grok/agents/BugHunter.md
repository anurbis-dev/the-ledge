...
name: BugHunter
description: >
  Use when: баг, ошибка, крах, null reference, race condition, утечка памяти, edge case,
  lifecycle issue, потекла память, найди баг. Defensive code analyst for the LEDGE
  (vanilla JS ES6, Canvas 2D platformer): spots stale runtime state, collide/step races,
  import-cycle hazards, persist/bake mismatches; suggests defensive fixes and registers
  Critical/High bugs as todos.
prompt_mode: full
model: inherit
